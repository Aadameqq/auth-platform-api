...
public class InitializePasswordResetCommandHandlerTests
{
    ...
    {
        var result = await commandHandler.Execute("invalid-email");
    ...
    {
        var result = await commandHandler.Execute(existingAccount.Email);
    ...
    public async Task WhenAccountWithGivenEmailExists_ShouldCreateCode()
    {
        ...
        await commandHandler.Execute(existingAccount.Email);
