...
public class UnassignRoleCommandHandlerTests
{
    ...
    {
        commandHandler = new UnassignRoleCommandHandler(accountsRepositoryMock.Object);
    ...
    {
        var result = await commandHandler.Execute(Guid.NewGuid(), Guid.NewGuid());
    ...
    {
        var result = await commandHandler.Execute(existingAccount.Id, existingAccount.Id);
    ...
    {
        var result = await commandHandler.Execute(Guid.NewGuid(), existingAccount.Id);
    ...
    public async Task ShouldPersistCorrectAccount_WhenAccountWithGivenIdExistsAndIsNotIssuer()
    {
        ...
        await commandHandler.Execute(Guid.NewGuid(), existingAccount.Id);
