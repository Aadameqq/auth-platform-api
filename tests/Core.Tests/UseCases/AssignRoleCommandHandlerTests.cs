...
//     {
//         commandHandler = new AssignRoleCommandHandler(accountsRepositoryMock.Object);
//
...
//     {
//         var result = await commandHandler.Execute(Guid.NewGuid(), Guid.Empty, Role.Admin);
//
...
//     {
//         var result = await commandHandler.Execute(existingAccount.Id, existingAccount.Id, Role.Admin);
//
//         Assert.True(result.IsFailure);
...
//
//         var result = await commandHandler.Execute(Guid.NewGuid(), existingAccount.Id, Role.Admin);
//
//         Assert.True(result.IsFailure);
...
//
//         await commandHandler.Execute(Guid.NewGuid(), existingAccount.Id, Role.Admin);
//
